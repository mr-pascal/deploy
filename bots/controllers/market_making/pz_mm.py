...
class PZMMConfig(MarketMakingControllerConfigBase):
    """
    Configuration required to run the D-Man Maker V2 strategy.
    """

    controller_name: str = "pz_mm"
...
class PZMM(MarketMakingControllerBase):
    def __init__(self, config: PZMMConfig, *args, **kwargs):
    ...
    def first_level_refresh_condition(self, executor):
        if self.config.top_executor_refresh_time is not None:
            if (
                self.get_level_from_level_id(executor.custom_info["level_id"])
                == 0
            ):
                return (
    ...
    def get_executor_config(
        ...
        amounts_quote = [
            amount * price for amount, price in zip(amounts, prices)
        ]
